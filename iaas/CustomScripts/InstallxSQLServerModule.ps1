...
function DownloadAndExecuteSqlScript($uri, $path){
	if ($uri -ne "") {
		# Download SQL Script
		Invoke-WebRequest -Uri $uri -OutFile $path
		# Execute SQL Script
		Invoke-sqlcmd -InputFile $path -Database $databaseName -Username $dbUserName -Password $password 
	}
}

if ($sqlSchemaScript -ne "" -or $sqlDataScript -ne "") {
	$scriptsPath = "c:\sqlScripts"
	
	mkdir $scriptsPath

	DownloadAndExecuteSqlScript($sqlSchemaScript, "$scriptsPath\Schema.ps1")
	DownloadAndExecuteSqlScript($sqlDataScript, "$scriptsPath\Data.ps1")
}